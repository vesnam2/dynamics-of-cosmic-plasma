...
dt = 0.01 # VREMENSKI KORAK (pozeljno da bude mali u odnosu na jedan ziro-period 2pim/qB) Za elektron dt=0.001, za proton i alfa česticu 0.1
...
Nkoraka = int(T_sim/dt) #Jer se dobije broj u formatu .0 i onda ne prepoznaje kao broj u arange fji
...
plot_geeks = ax.plot(Rx/d, Ry/d, Rz/d, color='slateblue') 

ax.set_title('Putanja u magnetnom polju B=(A*x + B0)e_z') 
ax.set_xlabel('x') 
ax.set_ylabel('y') 
ax.set_zlabel('z') 
plt.show()
...
plt.title('Promena kinetičke energije u magnetnom polju B=(A*x + B0)e_z')
plt.xlabel('t [s]')
...
#Odvojeno u xy i xz ravni
plt.subplot(1, 2, 1) 
plt.plot(Rx/d, Ry/d, color='slateblue')
plt.title('XY')
plt.xlabel('x')
plt.ylabel('y')
...
plt.subplot(1, 2, 2)
plt.plot(Rx/d, Rz/d, color='slateblue')
plt.title('XZ')
plt.xlabel('x')
plt.ylabel('z')
plt.tight_layout(4)
plt.savefig('Putanja_tela_u_magnetnom_polju.png')
plt.show()
